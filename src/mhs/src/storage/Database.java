...
import mhs.src.storage.persistence.remote.OAuth2;
 ...
 * Database
 * 
 * Database interfaces persistent the data storage mechanism, on local disk and remote (Google
 * Calendar Service).
 * 
 * - Handles task queries and CRUD operations - Handles user configuration setting operations
 * 
...
public class Database {
  ...
  private static final String EXCEPTION_MESSAGE_SYNCRONIZATION_WITH_REMOTE_STORAGE_FAILED =
      "Syncronization with remote storage failed.";
  private static final String EXCEPTION_MESSAGE_REMOTE_SYNC_NOT_ENABLED =
      "Remote Sync Not Enabled.";
  private static final String EXCEPTION_MESSAGE_NO_CONNECTIVITY_WITH_REMOTE_STORAGE =
      "No Connection with Remote Storage.";
  ...
  protected Database(String taskRecordFileName, boolean disableSyncronize)
    ...
    if (taskRecordFileName == null) {
      throw new IllegalArgumentException(String.format(EXCEPTION_MESSAGE_NULL_PARAMETER,
   ...
   * @throws ServiceException
   */
  private void initalizeDatabase(String taskRecordFileName) throws IOException {
  ...
  private void setSyncEndDateTime(int monthsBeforeNow) {
    syncEndDateTime = DateTime.now().plusMonths(monthsBeforeNow).toDateMidnight().toDateTime();
  }
  ...
  private void setSyncStartDateTime(int monthsAfterNow) {
    syncStartDateTime = DateTime.now().minusMonths(monthsAfterNow).toDateMidnight().toDateTime();
  }
   ...
   * @throws IOException
   */
  public void syncronizeDatabases() throws UnknownHostException, ServiceException {
    logEnterMethod("syncronizeDatabases");
    logger.log(Level.INFO, "Syncronizing Databases");
    if (!isRemoteServiceConnectivityActive()) {
      throw new UnknownHostException(EXCEPTION_MESSAGE_NO_CONNECTIVITY_WITH_REMOTE_STORAGE);
    }
    if (!isRemoteSyncEnabled) {
      throw new ServiceException(EXCEPTION_MESSAGE_REMOTE_SYNC_NOT_ENABLED);
    }
    boolean isSyncronizeSchedulingSuccessful = syncronize.syncronizeDatabases();
    if (!isSyncronizeSchedulingSuccessful) {
      syncronize.disableRemoteSync();
      throw new ServiceException(EXCEPTION_MESSAGE_SYNCRONIZATION_WITH_REMOTE_STORAGE_FAILED);
    }
   ...
   */
  boolean initializeGoogleCalendarService() throws IOException, AuthenticationException,
      ServiceException {
    ...
    googleCalendar =
        new GoogleCalendarMhs(GOOGLE_CALENDAR_APP_NAME, userGoogleAccount, userGoogleAuthToken);
    logExitMethod("initializeGoogleCalendarService");
    return true;
  }
  ...
  private String getSavedUserGoogleAuthToken() {
    if (!configFile.hasNonEmptyConfigParameter(CONFIG_PARAM_GOOGLE_AUTH_TOKEN)) {
      return null;
    }
    String savedUserGoogleAuthToken = configFile.getConfigParameter(CONFIG_PARAM_GOOGLE_AUTH_TOKEN);
  ...
  private String getSavedUserGoogleAccount() {
    if (!configFile.hasNonEmptyConfigParameter(CONFIG_PARAM_GOOGLE_USER_ACCOUNT)) {
      return null;
    }
    String savedUserGoogleAccount = configFile.getConfigParameter(CONFIG_PARAM_GOOGLE_USER_ACCOUNT);
   ...
   * 
   * @param userName user google account
   * @param userPassword user google account password
   * @throws Exception
   */
  public void loginUserGoogleAccount(String userName, String userPassword) throws Exception {
    logEnterMethod("loginUserGoogleAccount");
    OAuth2 oAuth2 = new OAuth2();
    //OAuth2 oAuth2 = OAuth2.getInstance();
    //oAuth2.initializeOAuth2Authentication();
   ...
   * @throws ServiceException
   */
  private String authenticateUserGoogleAccount(String userName, String userPassword)
      throws AuthenticationException, UnknownHostException, IOException, ServiceException {
    syncronize.disableRemoteSync();
    String googleAccessToken =
        GoogleCalendarMhs.retrieveUserToken(GOOGLE_CALENDAR_APP_NAME, userName, userPassword);
    googleCalendar = new GoogleCalendarMhs(GOOGLE_CALENDAR_APP_NAME, userName, googleAccessToken);
  ...
  public String getUserGoogleAccountName() {
    ...
    if (configFile.hasConfigParameter(CONFIG_PARAM_GOOGLE_USER_ACCOUNT)) {
      String[] googleUserAccountString =
          configFile.getConfigParameter(CONFIG_PARAM_GOOGLE_USER_ACCOUNT).split(
  ...
  /**
   * Saves user Google Calendar Service access token and user google account email to config file
   * 
   ...
   * @throws IOException
   */
  synchronized void saveGoogleAccountInfo(String googleUserAccount, String googleAuthToken)
      throws IOException {
    logEnterMethod("saveGoogleAccountInfo");
    if (googleAuthToken != null) {
      configFile.setConfigParameter(CONFIG_PARAM_GOOGLE_AUTH_TOKEN, googleAuthToken);
    }
    if (googleUserAccount != null) {
      configFile.setConfigParameter(CONFIG_PARAM_GOOGLE_USER_ACCOUNT, googleUserAccount);
    }
    configFile.save();
  ...
  public Task query(int taskId) throws TaskNotFoundException {
    ...
    if (!taskLists.containsTask(taskId)) {
      throw new TaskNotFoundException(EXCEPTION_MESSAGE_TASK_DOES_NOT_EXIST);
    }
    logExitMethod("query");
  ...
  /**
   * Return tasks with matching taskName, case-insensitive substring search (exclusive of deleted
   * tasks)
   * 
   * @param orderByStartDateTime
   * @param taskName
  ...
  /**
   * Returns tasks that match specified TaskCategory (exclusive of deleted tasks)
   * 
   * @param orderByStartDateTime
   * @param queryTaskCategory
   * @return list of matched tasks
   */
  public List<Task> query(TaskCategory queryTaskCategory, boolean orderByStartDateTime) {
    logEnterMethod("query");
  ...
  /**
   * Returns tasks that is within startDateTime or endDateTime inclusive (exclusive of deleted
   * tasks)
   * 
   * @param orderByStartDateTime
   * @param startDateTime
  ...
  public List<Task> query(DateTime startDateTime, DateTime endDateTime,
    ...
    logExitMethod("query");
    return taskLists.getTasks(startDateTime, endDateTime, includeFloatingTasks,
        orderByStartDateTime);
  }

  /**
   * Returns task that matches any of the specified parameters (exclusive of deleted tasks)
   * 
   * @param taskName
   * @param startDateTime
   * @param endDateTime
   ...
   */
  public List<Task> query(String taskName, DateTime startDateTime, DateTime endDateTime,
    ...
    logExitMethod("query");
    return taskLists.getTasks(taskName, startDateTime, endDateTime, orderByStartDateTime);
  }
   ...
   * @return list of matched tasks
   */
  public List<Task> query(String taskName, TaskCategory taskCategory, DateTime startDateTime,
    ...
    logExitMethod("query");
    return taskLists.getTasks(taskName, taskCategory, startDateTime, endDateTime,
        orderByStartDateTime);
  }

  /**
   * Adds a task to local storage and remote (if authenticated and connection exists)
   * 
   * @param task
  ...
  public Task add(Task task) throws InvalidTaskFormatException, IOException,
    ...
    if (!TaskValidator.isTaskValid(task)) {
      throw new InvalidTaskFormatException(EXCEPTION_MESSAGE_INVALID_TASK_FORMAT);
    }
   ...
   * Creates task to add
   * 
   * Updates the following attributes: - unique taskId - TaskCreated - TaskUpdated
   * 
   * Sets the following attributes to new task default: - TaskLastSync - gCalTaskId
   * 
   * @param taskToAdd
   */
  ...
  void deleteTaskInTaskList(Task taskToDelete) {
    ...
    setUpdatedTimeAheadToForcePush(taskToDelete, SYNC_FORCE_PUSH_UPDATED_DATE_TIME_AHEAD_VALUE);
   ...
   * @throws ServiceException
   */
  public Task update(Task updatedTask) throws NullPointerException, IOException, ServiceException,
   ...
   * @throws TaskNotFoundException
   */
  private void preserveTaskNonEditableFields(Task updatedTaskToSave) throws TaskNotFoundException {
  ...
  private void updateTaskinTaskList(Task updatedTaskToSave) {
    ...
    setUpdatedTimeAheadToForcePush(updatedTaskToSave, SYNC_FORCE_PUSH_UPDATED_DATE_TIME_AHEAD_VALUE);
   ...
   */
  public void waitForAllBackgroundTasks(int maxExecutionTimeInSeconds) throws InterruptedException,
  ...
  public void clearRemoteDatabase() throws IOException, ServiceException {
    ...
    if (isRemoteSyncEnabled) {
      try {
        googleCalendar.deleteEvents(DateTime.now().minusYears(1).toString(), DateTime.now()
   ...
   * @throws IOException
   */
  public boolean setConfigParameter(String configParameterToSet, String configParameterValueToSet)
      throws IOException {
    ...
    configFile.setConfigParameter(configParameterToSet, configParameterValueToSet);
   ...
   * 
   * @param configParameter
   * @return true if configuration parameter is removed successfully or false if it does not exist
   * @throws IOException
   */
  public boolean removeConfigParameter(String configParameter) throws IOException {
